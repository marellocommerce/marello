...
namespace Marello\Bundle\RMABundle\Tests\Functional\Controller\Api\Rest;
...
class ReturnControllerTest extends WebTestCase
{
    ...
    {
        /** @var OrderInterface $returnedOrder */
        $returnedOrder = $this->getReference('marello_order_unreturned');

        $data = [
            'order'       => $returnedOrder->getOrderNumber(),
            'returnItems' => $returnedOrder->getItems()->map(function (OrderItemInterface $item) {
                return [
        ...
        $orderedItemIds = $returnedOrder->getItems()->map(function (OrderItemInterface $orderItem) {
            return $orderItem->getId();
        });

        $returnedItemIds = $return->getReturnItems()->map(function (ReturnItemInterface $returnItem) {
        ...
        $return->getReturnItems()->map(function (ReturnItemInterface $returnItem) {
            $this->assertEquals(
